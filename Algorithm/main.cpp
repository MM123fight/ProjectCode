...
int main() {
    ...
    const std::string all_experiments[10] = {"Mat100|100_1000_0.01_0.5", "Mat0|1000_10000_0.01_0.5", "test1", "test3",
    ...
    ProbData<unsigned int, double> *inst = new ProbData<unsigned int, double>(root_directory, experiment, "LPtoLS");
    //inst->data_print();
    double alpha = 0.;
    ...
    precision = 1e-3;
    std::vector<double> zeros(inst->n, 0);
    /*
    ...
    */
    ...
    AbsAlgorithm<unsigned int, double> *method = new PCDM<unsigned int, double>(inst, 1, 1);
    method->set_loss(0., inst->b, zeros);
    //method->set_loss(inst->b);
    //method->set_reg(1.);
    method->solver(x, tau, 100000, precision, 1, true);
